...
function ResultsPage() {
  ...
  const onDownload = async () => {
    ...
    try {
      const filename = buildPdfFilename(lessonTitle, progress.studentName);
      ...
      <div className="max-w-5xl mx-auto px-4 py-8 md:py-12">
        ...
        <div dir="rtl" className="space-y-6 bg-background/0">
          ...
          <div className="glass-panel rounded-3xl p-6 md:p-8 text-center">
            ...
            <h1 className="text-3xl md:text-4xl font-extrabold gold-text mb-2">
              {lessonTitle}
            </h1>
            ...
            <div className="grid sm:grid-cols-2 gap-3 max-w-xl mx-auto text-right">
              ...
              <Field
                label="رقم الجلسة"
                value={progress.sessionId?.slice(0, 8) || "—"}
              />
            </div>
          </div>
          ...
          {results.map((r) => (
            <div key={r.section.id} className="space-y-4">
              <h2 className="text-2xl font-bold electric-text">
                تصحيح: {r.section.title}
              </h2>

              {r.section.questions.map((q, idx) => {
                const score = r.perQuestion.find(
                  (p) => p.questionId === q.id
                );
                const lines = resolveAnswer(
                  q,
                  progress.answers[r.section.id]?.[q.id]
                );
                  ...
                  <div key={q.id} className="glass-panel rounded-2xl p-5">
                    <div className="flex items-start justify-between gap-3 mb-3 flex-wrap">
                      <h3 className="text-arabic-lg font-bold flex-1">
                        <span className="electric-text">سؤال {idx + 1}.</span>{" "}
                        {q.prompt}
                      </h3>
          ...
          <div className="glass-panel rounded-3xl p-6">
            ...
            <div className="space-y-4">
              <div className="rounded-2xl p-4 border border-accent/30 bg-secondary/30">
                <div className="font-semibold mb-2 electric-text">
                  في التَّعبيرِ
                </div>
                <p className="text-arabic-lg leading-loose whitespace-pre-wrap">
                  {progress.expressionText?.trim() || "— لم يُجِب —"}
                </p>
              </div>
              ...
              <div className="rounded-2xl p-4 border border-accent/30 bg-secondary/30">
                ...
                <p className="text-arabic-lg leading-loose whitespace-pre-wrap">
                  {progress.researchText?.trim() || "— لم يُجِب —"}
                </p>
              </div>
            </div>
          </div>
...
function AnswerLine({
  ...
  line: ReturnType<typeof resolveAnswer>[number];
}) {
  const ok = line.ok;
  const isOk = ok === true;
    ...
    >
      ...
      <div className="grid sm:grid-cols-2 gap-3 text-sm">
        <div>
          ...
          <div className={`font-semibold ${isWrong ? "text-destructive" : ""}`}>
            {line.given}
          </div>
        </div>
...
function buildPdfFilename(textTitle: string, studentName: string) {
  ...
  const title = clean(textTitle, "النص");
  const student = clean(studentName, "تلميذ");

  return `${title}_${student}.pdf`;
}